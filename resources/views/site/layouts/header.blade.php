      ...
      @auth
          <li class="nav-item dropdown relative">
              ...
              <ul class="dropdown-menu dropdown-menu-end absolute right-0 mt-2 bg-white border rounded shadow-md min-w-[200px]">
                  <li><span class="dropdown-item-text px-4 py-2 block text-gray-700">{{ Auth::user()->email }}</span></li>
                  <li><hr class="dropdown-divider border-gray-200"></li>
                  <li>
                      <form method="POST" action="{{ route('logout') }}">
                          @csrf
                          <button class="dropdown-item w-full text-left px-4 py-2 hover:bg-gray-100">Sair</button>
                      </form>
                  </li>
              </ul>
          </li>
